...
/**
 * Recompute shade_signatures aggregate fields (embedding, base_color_lab, flake attrs)
 * by averaging across all image_signatures rows for this shade.
 */
async function recomputeShadeAggregate(shadeId: number): Promise<void> {
  const supabase = getSupabase();
  ...
  // Average embedding (parse from string when pgvector returns it as text)
